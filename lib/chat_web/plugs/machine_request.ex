defmodule ChatWeb.Plugs.MachineRequest do
  use Plug.Builder
  ...
  def handle_conn(%Plug.Conn{params: params} = conn, _opts) do
    ...
    cookie_id = Map.get(conn.req_cookies, @machine_id_key, machine_id)

    cond do
      param_id && param_id == machine_id ->
        Logger.info("Correct machine based on parameter #{machine_id}. Set cookie and let pass.")
        put_resp_cookie(conn, @machine_id_key, machine_id, max_age: @cookie_ttl)
      ...
      cookie_id && cookie_id == machine_id ->
        Logger.info("Correct machine #{machine_id} and cookie. Let pass.")
        conn
